...
class TaskRepository(AbstractRepository):  # inherits Abstractrepository
    ...
    def remove(self, project_id:int, task_id: int):
        remove_task = session.query(Task).filter(Task.id == task_id, Task.project_id == project_id).first()
        session.delete(remove_task)
        session.commit()
        return remove_task

    def update(self,project_id:int ,task_id: int, new_task: str):
        update_task = session.query(Task).filter(Task.id == task_id, Task.project_id == project_id).first()
        if update_task:
